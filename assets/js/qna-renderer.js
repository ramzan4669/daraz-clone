...
function renderQuestionItem(item, index) {
  ...
  const questionTime = item.questionTime || '';
  const answer = item.answer ? item.answer : '';
  const sellerName = item.sellerName || '';
  const answerTime = item.answerTime || '';
...
function renderPopulatedState(items, sellerName, totalItems) {
  const listHtml = items.map(function (item, idx) {
    return renderQuestionItem(item, idx);
  }).join('');

  const sectionTitle = sellerName
...
function renderQnA(product) {
  ...
  const titleEl = document.querySelector('#qna-section .product-info-section-title.outer-title');
  if (!titleEl) return;

  const questions = product && product.questions;
  const items = questions && Array.isArray(questions.items) ? questions.items : [];
  const totalItems = questions && typeof questions.totalItems === 'number' ? questions.totalItems : items.length;

  titleEl.textContent = 'Questions about this product (' + totalItems + ')';

  if (totalItems > 0 && items.length > 0) {
    const firstAnswer = items[0];
    const sellerName = firstAnswer && firstAnswer.sellerName ? firstAnswer.sellerName : (product.seller || '');
